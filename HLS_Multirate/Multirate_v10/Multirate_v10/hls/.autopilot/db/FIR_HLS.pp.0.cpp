...
static accu_data_t H_accu_FIR_kernel[117];
...
#pragma HLS DATAFLOW
 ...
 static short mod_value=0;
    ...
    if (mod_value==0) {
        kernel_out.write(FIR_filter(H_accu_FIR_kernel, b_FIR_kernel, 117, dec_out.read()));
        mod_value=3;
    }
    else {
        mod_value--;
...
void DECIMATOR(hls::stream<fir_data_t> &in, hls::stream<fir_data_t> &out){
    ...
    if (mod_value==0) {
        y1_phase0 = FIR_filter(H_accu_FIR_dec_40, b_FIR_dec_int_40, 6, in.read());
        out.write( y1_phase0 + y1_phase1 + y1_phase2 + y1_phase3);
        mod_value=1;
    }
    else if (mod_value==1) {
        y1_phase1 = FIR_filter(H_accu_FIR_dec_43, b_FIR_dec_int_43, 5, in.read());
        mod_value=2;
    }
    else if (mod_value==2) {
        y1_phase2 = FIR_filter(H_accu_FIR_dec_42, b_FIR_dec_int_42, 5, in.read());
        mod_value=3;

    }
    else {
        y1_phase3 = FIR_filter(H_accu_FIR_dec_41, b_FIR_dec_int_41, 5, in.read());
        mod_value=0;
    }
...
void INTERPOLATOR(hls::stream<fir_data_t> &in, hls::stream<fir_data_t> &out){
    ...
    if (mod_value==0) {
        y2 = in.read();
        out.write(FIR_filter(H_accu_FIR_int_40, b_FIR_dec_int_40, 6, y2)*4);
        mod_value=1;
    }
    else if (mod_value==1) {
        out.write(FIR_filter(H_accu_FIR_int_41, b_FIR_dec_int_41, 5, y2)*4);
        mod_value=2;
    }
    else if (mod_value==2) {
        out.write(FIR_filter(H_accu_FIR_int_42, b_FIR_dec_int_42, 5, y2)*4);
        mod_value=3;

    }
    else {
        out.write( FIR_filter(H_accu_FIR_int_43, b_FIR_dec_int_43, 5, y2)*4);
        mod_value=0;
    }
